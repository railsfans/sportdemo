...
def login 
    ...
    respond_to do |format|
		if flag && usertype=="student"
			format.json { render :json=>{:usertype=>user.first.usertype, :userdata=>{:email=>Login.getuserinfo(user.first.id).first.email, :sex=>Login.getuserinfo(user.first.id).first.sex==true ? 'male' : 'female', :phone=>Login.getuserinfo(user.first.id).first.phone, :height=>Login.getuserinfo(user.first.id).first.height, :weight=>Login.getuserinfo(user.first.id).first.weight, :name=>Login.getuserinfo(user.first.id).first.name, :studentid=>Login.getuserinfo(user.first.id).first.studentid, :class=>'class 1'}, :passwdtoken=>passwdtoken, :current=>{ :step=>"3133", :distance=>"4.31", :calorie=>"672", :activetime=>"53" },:target=>{:step=>user.first.target.step.round(0), :distance=>user.first.target.distance.round(2), :calorie=>user.first.target.calorie.round(0), :activetime=>user.first.target.activetime.round(0) }} }
