...
#			format.json { render :json=>{:usertype=>user.first.usertype, :userdata=>{:email=>Login.getuserinfo(user.first.id).first.email, :sex=>Login.getuserinfo(user.first.id).first.sex==true ? 'male' : 'female', :name=>Login.getuserinfo(user.first.id).first.name}, :passwdtoken=>passwdtoken, :classinfo=>Login.getuserinfo(user.first.id).first.shclasses.collect { |list| { :classinfo=>{:classname=>list.name, :studentinfo=>Student.where(:shclass_id=>list.id)}}} } }
			...
			format.json { render :json=>{:usertype=>user.first.usertype, :userdata=>{:email=>Login.getuserinfo(user.first.id).first.email, :sex=>Login.getuserinfo(user.first.id).first.sex==true ? 'male' : 'female', :name=>Login.getuserinfo(user.first.id).first.name, :phone=>Login.getuserinfo(user.first.id).first.phone, :gradeteacherid=>Login.getuserinfo(user.first.id).first.gradeteacherid}, :passwdtoken=>passwdtoken, :grade=>Login.getuserinfo(user.first.id).first.shgrades.first.shclasses.collect { |list| { :class=>list.name}} } }
		else
       		format.json	{ render :json=>{:errormessage=>message}}
       	end
		end
