...
def getexercise
	...
	respond_to do |format|
		if flag
			format.json  { render :json=> {:success=>flag, :current=>{ :step=>exercisedata.sum(:step).to_i, :distance=>exercisedata.sum(:distance), :calorie=>exercisedata.sum(:calorie).to_i, :activetime=>exercisedata.sum(:step).to_i } } }
		else
